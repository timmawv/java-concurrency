                ...
                if (!isParkingFull()) {
                    ...
                    try {
                        ...
                        while (isParkingFull()) {
                            ...
                            condition.await();
                            //при такой реализации поток, машина которая сюда попала не сможет никогда зайти на парковку.
                            //Потому что когда она просыпается она пытается захватить лок, если захватила освобождает его.
                            //1 машина которая покинула парковку уже 100 раз зайдет и снова займет место, а это снова прийдет и будет ждать.
                        }
                    } finally {
                        reentrantLock.unlock();
                    }
